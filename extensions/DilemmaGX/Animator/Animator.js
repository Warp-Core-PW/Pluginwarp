...
class Animator {
  ...
  constructor(runtime) {
    ...
    this._formatMessage = runtime.getFormatMessage({
      "zh-cn": {
        ...
        "Ani.Animate": "用[type]方式计算时长[length]从[start]到[end]的函数于[time]时的值", //block
      ...
      en: {
        "Ani.extensionName": "Animator",
        "Ani.Animate": "[type](start=[start], end=[end], duration=[length]) at[time]second(s)", //block
  ...
  Move({ type, endx, endy, time }, util) {
    if (util.stackFrame.startTime !== undefined) {
      ...
      ] = [type, Date.now(), parseFloat(time) * 1000, util.target.x, util.target.y, parseFloat(endx), parseFloat(endy)];
  ...
  Effect({ type, endv, Effect, time }, util) {
    if (util.stackFrame.startTime !== undefined) {
      ...
      ] = [type, Date.now(), parseFloat(time) * 1000, util.target.effects[Effect], parseFloat(endv)];
      if (util.stackFrame.length <= 0) {
        util.target.setEffect(Effect, util.stackFrame.endV);
        return;
      }
      ...
      [
        ...
        util.stackFrame.startV,
        util.stackFrame.endV,
      ] = [type, Date.now(), parseFloat(time) * 1000, util.target.size, parseFloat(endv)];
      if (util.stackFrame.length <= 0) {
        util.target.setSize(util.stackFrame.endV);
        return;
      }
  ...
  Dir({ type, endv, time }, util) {
    if (util.stackFrame.startTime !== undefined) {
      ...
      [
        ...
        util.stackFrame.startV,
        util.stackFrame.endV,
      ] = [type, Date.now(), parseFloat(time) * 1000, util.target.direction, parseFloat(endv)];
      if (util.stackFrame.length <= 0) {
        util.target.setDirection(util.stackFrame.endV);
        return;
      }
