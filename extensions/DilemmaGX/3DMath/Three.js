...
class Three {
    ...
    constructor(runtime) {
        ...
        this._formatMessage = runtime.getFormatMessage({
            'zh-cn': {
                ...
                'Three.block.normV3': 'V3: 标准化[a]',
                'Three.block.normV2': 'V2: 标准化[a]',
    ...
    getInfo() {
        const blocks = [
            ...
            },
            {
                ...
                text: this.formatMessage('Three.block.getAxisOfV3'),
                arguments: {
                    axis: { type: 'string', menu: 'get3DAxis' },
                    vector: {
                        type: 'string',
                        defaultValue: '[0,0,0]',
                    },
            ...
            },
            {
                ...
                opcode: 'normV3',
                blockType: 'reporter',
                text: this.formatMessage('Three.block.normV3'),
                arguments: {
                    a: { type: 'string', defaultValue: '[1,2,3]' },
                },
            },
            {
                disableMonitor: true,
            ...
            },
            {
                ...
                opcode: 'normV2',
                blockType: 'reporter',
                text: this.formatMessage('Three.block.normV2'),
                arguments: {
                    a: { type: 'string', defaultValue: '[1,2]' },
                },
            },
            {
                disableMonitor: true,
     ...
     * @param {boolean} singleValueAllowed 是否允许从单值转换为数组，如5=>[5,5,5]
     * @returns {Array|null} 长为len的array
     */
    getArrayFromParam(param, len, copy = false, singleValueAllowed = false) {
        let arr = null;
        ...
        } else if (singleValueAllowed) {
            const v = Cast.toNumber(param);
            if (len === 2) return [v, v];
            return [v, v, v];
        }
        if (arr) {
    ...
    getAxisOfV3New(args) {
        ...
        return this.__get3DAxis(vector, axis);
    }
    ...
    addV3(args) {
        const a = this.getArrayFromParam(args.a, 3, false, true);
        const b = this.getArrayFromParam(args.b, 3, false, true);
        if (a && b) {
    ...
    normV3(args) {
        ...
        if (a) {
            ...
            if (mag === 0) return new SafeVector([0, 0, 0]);
            return new SafeVector([a[0] / mag, a[1] / mag, a[2] / mag]);
        }
        return new SafeVector([0, 0, 0]);
    }
    ...
    dotProductOfV2(args) {
        const a = this.getArrayFromParam(args.a, 2, false, true);
        const b = this.getArrayFromParam(args.b, 2, false, true);
        if (a && b) {
    ...
    normV2(args) {
        ...
        if (a) {
            ...
            if (mag === 0) return new SafeVector([0, 0]);
            return new SafeVector([a[0] / mag, a[1] / mag]);
        }
        return new SafeVector([0, 0]);
    }
    ...
    cam3DgetOnePosition({ a }) {
        const axis = Cast.toNumber(a);
        return this.__get3DAxis(camera.position, axis);
    }

    cam3DgetOneRotation({ a }) {
        const axis = Cast.toNumber(a);
        return this.__get3DAxis(camera.rotation, axis);
    }
    ...
    spr3DgetPositionComponent({ component }, util) {
        ...
        const data = this.getTarget3DData(target);
        return this.__get3DAxis(data.position, idx);
    }
    ...
    spr3DgetRelativePositionComponent({ component }, util) {
        ...
        const data = this.getTarget3DData(target);
        this._updateTargetRelativeCoords(data);
        return this.__get3DAxis(data.relative, idx);
